...
let db={users:[],cdks:[],admins:[],config:{version:"4.0.1"}};
...
http.createServer(function(req,res){
  ...
  req.on("end",function(){
    ...
    if(url==="/"){r={status:"ok",version:"4.0.1",users:db.users.length,time:new Date().toISOString()}}else var r={error:"Unknown endpoint"};
    if(url==="/api/health")r={status:"ok",version:"4.0.1",users:db.users.length,time:new Date().toISOString()};
...
}).listen(PORT,function(){console.log("StarOrbi Server v4.0.1 on http://localhost:"+PORT)});
